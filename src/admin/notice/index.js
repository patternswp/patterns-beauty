...
/* Local */
class PatternsBeautySettingsApi {
    constructor() {
        this.endpoint = 'patterns-beauty/v1/settings';
        this.nonce = PatternsBeautyLocalize.nonce;
        this.getSettings();
        this.bindEvents();
    }
    ...
    bindEvents() {
        document.addEventListener('click', (event) => {
            if (event.target.classList.contains('patterns-beauty-gsn-close')) {
                this.modifySettings('hide_get_started_notice', true);
            }
            else if (event.target.classList.contains('patterns-beauty-rn-permanent-close')) {
                this.modifySettings('remove_review_notice_permanently', true);
            }
            else if (event.target.classList.contains('patterns-beauty-rn-temporary-close')) {
                this.modifySettings('remove_review_notice_temporary_date_time', 1);
            }
            else if (event.target.classList.contains('patterns-beauty-install-plugins')) {
                event.target.classList.add("patterns-beauty-processing");
                this.installPlugins();
            }
        });
    }
    ...
    updateUI() {
        if (this.settings.hide_get_started_notice) {
            const noticeElement = document.getElementById('patterns-beauty-gsn');
            if (noticeElement) {
        ...
        if (!this.isLoadReview()) {
            const reviewNoticeElement = document.getElementById('patterns-beauty-review-notice');
    ...
    async installPlugins() {
        const plugins = PatternsBeautyLocalize.recommended_plugins;
        if (plugins && plugins.length) {
            for (const plugin of plugins) {
                try {
                    await apiFetch({
                        path: 'patterns-beauty/v1/install-plugin',
                        method: 'POST',
                        data: plugin,
                        headers: {
                            'X-WP-Nonce': this.nonce
        ...
        window.location.replace(PatternsBeautyLocalize.theme_info_url);
    }
}

document.addEventListener('DOMContentLoaded', () => {
    new PatternsBeautySettingsApi();
});
