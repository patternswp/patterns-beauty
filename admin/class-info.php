 ...
 * @subpackage Patterns_Beauty/Patterns_Beauty_Info
 */
...
class Patterns_Beauty_Info {
	...
	public function add_theme_menu() {
		$this->hook_suffix[] = add_theme_page( esc_html__( 'Theme Info', 'patterns-beauty' ), esc_html__( 'Theme Info', 'patterns-beauty' ), 'edit_theme_options', PATTERNS_BEAUTY_THEME_NAME, array( $this, 'info_screen' ) );
	}
	...
	public function enqueue_resources( $hook_suffix ) {
		...
		$unique_id = PATTERNS_BEAUTY_THEME_NAME . '-info';
		...
		$deps_file = PATTERNS_BEAUTY_PATH . 'build/admin/info/info.asset.php';
		...
		wp_enqueue_script( $unique_id, PATTERNS_BEAUTY_URL . 'build/admin/info/info.js', $dependency, $version, true );

		wp_enqueue_style( $unique_id, PATTERNS_BEAUTY_URL . 'build/admin/info/info.css', array(), $version );
		wp_style_add_data( $unique_id, 'rtl', 'replace' );

		/* Localize */
		$localize = apply_filters(
			'patterns_beauty_info_localize',
			array(
				'version'  => $version,
...
/**
 * Return instance of Patterns_Beauty_Info class
 *
 * @since 1.0.0
 *
 * @return Patterns_Beauty_Info
 */
function patterns_beauty_info() { //phpcs:ignore
	return Patterns_Beauty_Info::instance();
}
patterns_beauty_info()->run();
